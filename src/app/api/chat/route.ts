import { streamText, tool, convertToModelMessages, safeValidateUIMessages } from "ai";
...
export async function POST(req: Request) {
  ...
  const validated = await safeValidateUIMessages({
    messages: (json as Record<string, unknown>).messages,
  });

  if (!validated.success) {
    return Response.json(
      { error: "Invalid messages format", details: validated.error.message },
      { status: 400 }
    );
  }
