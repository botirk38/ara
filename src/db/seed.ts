...
function buildCustomers(now: string) {
  return [
    {
      id: "cust-001",
...
function buildInvoices(now: string) {
  return [
    {
      id: "inv-001",
      customerId: "cust-001",
...
function buildSpecter(now: string) {
  return [
    {
      id: uuid(),
      customerId: "cust-001",
...
async function doSeed() {
  ...
  const seedCustomers = buildCustomers(now);
  const seedInvoices = buildInvoices(now);
  const seedSpecter = buildSpecter(now);
